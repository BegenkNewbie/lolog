...
func (s *slogLogger) init() {
	var slogs multiSlog
	for _, w := range s.wr {
		switch w.GetOutput() {
		case CONSOLE:
			opt := &slog.HandlerOptions{Level: toSlogLevel(w.Level())}
			slogs.loggers = append(slogs.loggers, slog.New(slog.NewTextHandler(w.GetWriter(), opt)))

		case FILE, CONTAINER:
			opt := &slog.HandlerOptions{Level: toSlogLevel(w.Level())}
			slogs.loggers = append(slogs.loggers, slog.New(slog.NewJSONHandler(w.GetWriter(), opt)))
		}
	}
	s.log = &slogs
}

func (s *slogLogger) Flush() {
	for _, w := range s.wr {
		w.Flush()
	}
}
